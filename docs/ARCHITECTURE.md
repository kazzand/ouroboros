# Ouroboros v6.81.0 — Architecture & Reference
...
hard max 50). A configured depth of **0 disables delegation entirely** — both the tool-side
...
one identity collects observations from EVERY surface (the shipped `claude-fable-5` is
both the scope reviewer and a triad slot), so a run of doc-only commits whose
...
| OUROBOROS_REVIEW_MODELS | anthropic/claude-fable-5,openai/gpt-5.6-sol,google/gemini-3.5-flash | Ordered reviewer slots shared by triad/plan/task/skill review; duplicate model IDs are independent slots |
