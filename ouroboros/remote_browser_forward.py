...
class SSHBrowserForwardManager:
    ...
    def __init__(
        ...
        *,
        ...
        # Append-only process custody snapshot: panic reads it without taking
        # the ordinary manager lock, including a child between spawn and
        # publication in ``_pending``.
        self._panic_processes: list[subprocess.Popen[Any]] = []
        ...
        *,
        ...
        for _attempt in range(self._max_bind_attempts):
            ...
            try:
                ...
                self._panic_processes.append(process)
                if self._closed:
                    if (
    ...
    def close_task(self, task_id: str) -> int:
        ...
        return _terminate_many(live, pending)
    ...
    def panic_close_all(self) -> int:
        ...
        self._closed = True
        processes = tuple(self._panic_processes)
