    ...
    *,
    ...
    if probe.returncode != 0 or raw_probe.returncode != 0:
        raise _error(
            ...
            phase="connect",
            details={"stderr": _safe_text(probe.stderr)},
        )
    resolved_configs: list[dict[str, list[str]]] = []
    for resolved_text in (probe.stdout, raw_probe.stdout):
        result: dict[str, list[str]] = {}
        for line in resolved_text.splitlines():
            key, separator, value = line.partition(" ")
            if separator:
                result.setdefault(key.strip().lower(), []).append(value.strip())
        resolved_configs.append(result)
    config, raw_config = resolved_configs
    ...
    if any(value and value.lower() != "none" for value in raw_config.get("setenv", [])):
        raise _error(
            "unsafe_ssh_environment",
            "The SSH alias configures SetEnv; execd refuses ambient remote variables.",
            phase="connect",
        )
    patterns = [token for value in raw_config.get("sendenv", []) for token in value.split()]
    for env_name in _SSH_ENV_KEYS:
        selected = False
    ...
    unsafe_forward_keys = ("localforward", "remoteforward", "dynamicforward", "tunnel")
    configured_forward_keys = {
        key
        for key in unsafe_forward_keys
        if any(
            value and value.lower() not in {"none", "false", "no"}
            for value in raw_config.get(key, [])
        )
    }
    if configured_forward_keys and forwarding:
        raise _error(
            "unsafe_ssh_forwarding",
            "The SSH alias contains forwarding or tunnel directives.",
            phase="connect",
        )
    if configured_forward_keys and (
        (config.get("clearallforwardings") or ["no"])[-1].lower()
        not in {"yes", "true"}
        or (config.get("tunnel") or ["no"])[-1].lower()
    ...
    if configured_forward_keys:
        _LOG.warning(
            "Ignored SSH alias forwarding directives for execd protocol: %s",
            ",".join(sorted(configured_forward_keys)),
        )
        config["_ouroboros_warning_directives"] = sorted(
            configured_forward_keys
        )
...
def validated_ssh_base_command(
    ...
    *,
    ...
    command, _config = _ssh_config(alias, ssh_binary, forwarding=forwarding)
    return command


def _minimal_ssh_env() -> dict[str, str]:
    return {key: os.environ[key] for key in _SSH_ENV_KEYS if os.environ.get(key)}
...
class OpenSSHExecdTransport:
    ...
    def __init__(self, request: Any) -> None:
        ...
        self._ssh_base, resolved_config = _ssh_config(
            self.alias,
            request.ssh_binary,
            forwarding=False,
        )
            ...
            [
                {
                    ...
                    "directives": list(warning_directives[:4]),
                }
            ]
    ...
    def health(self) -> dict[str, Any]:
        ...
        return {
            ...
            "error": _safe_text(self._reader_error) if self._reader_error else "",
            "warnings": [
                dict(row) for row in getattr(self, "_warnings", [])
            ],
        }

    def renew_lease(self, message: Mapping[str, Any]) -> None:
        self._renew_lease(str(message.get("task_id") or ""))
            ...
            while True:
                ...
                if self._reader_error is not None:
                    raise _error(
                        ...
                        retryable=True,
                        details={
                            "reason": _safe_text(self._reader_error),
    ...
    def _platform_probe(self, timeout_sec: float) -> dict[str, Any]:
        ...
        if machine is None:
            raise _error(
                "remote_platform_unsupported",
                f"Unsupported remote architecture: {_safe_text(fields[1])}",
                phase="connect",
            )
        ...
        if completed.returncode != 0:
            raise _error(
                ...
                details={
                    ...
                    "stderr": _safe_text(completed.stderr.decode("utf-8", errors="replace")),
                },
            )
        return completed
    ...
    def _stderr_text(self) -> str:
        with self._condition:
            return _safe_text(self._stderr_carry)
