...
- Any model-issued request, through shell, CLI, browser navigation or browser-evaluate JavaScript, to `/api/owner/connections` or any child path, for any HTTP method. Connection aliases, trust/retrust, bootstrap, reconnect and retirement are owner-controlled and must be performed in Settings → Connections or by the owner through the controlling-terminal CLI; the agent receives only the sealed placement evidence attached to its task and never administers the connection itself.
