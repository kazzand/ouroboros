...
def test_nonce_preamble_is_bounded_and_never_resynchronizes_after_false_magic():
    ...
    noncanonical = preamble.replace(b"\0" + b"1.0" + b"\0", b"\0" + b"01.00" + b"\0")
...
def test_control_shapes_versions_sequences_and_bulk_credit_fail_closed():
    ...
    with pytest.raises(ProtocolError, match="unknown required fields"):
        encode_control(
            {
                "kind": "panic",
                "seq": 0,
...
def test_reconnect_wire_reset_retains_operations_but_clears_session_state():
    ...
    transport._process = ExitedProcess()
    transport._reader = transport._stderr_reader = transport._lease_thread = None
    transport._condition = threading.Condition()
    transport._downloads = {"blob-a": {"event": pending, "error": None}}
    transport._download_current = "blob-a"
    transport._nonce = b"old"
    transport._sequence = 19
    transport._messages = deque([{"kind": "stale"}])
    transport._receive_sequence = ControlSequence()
    transport._reader_error = RuntimeError("old child")
    transport._stderr = bytearray(b"old")
    transport._stderr_carry = "api_key=old-secret"
    transport._handshake = {"session_id": "old"}
    transport._known_operations = {("request-a", "operation-a"): "a" * 64}
    transport._active_tasks = {"task-a"}

    transport._reset_wire_state()

    assert pending.is_set()
    assert transport._process is None
    assert transport._sequence == 0
    assert transport._messages == deque()
    assert transport._reader_error is None
    assert transport._handshake is None
