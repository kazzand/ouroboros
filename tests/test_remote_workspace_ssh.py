...
no-Python bundle/bootstrap compatibility remains a separate artifact-build
...
class _ExecdClient:
    ...
    def renew(self, task_id: str = "", ttl_ms: int = 15_000) -> None:
        fields: dict[str, Any] = {
...
def test_rws_108_default_broker_bootstrap_admit_read_cancel_and_panic(
    docker_ssh_host,
):
    ...
    attachment_payload = b"remote admission attachment\n"
    attachment_sha = hashlib.sha256(attachment_payload).hexdigest()
    attachment_manifest = [
        {
            "attachment_id": "broker-attachment",
            "label": "input.txt",
            "root": "artifact_store",
    ...
    try:
        ...
        admitted = broker.admit_workspace(
            ...
            attachment_blobs={attachment_sha: attachment_payload},
        )
        workspace_ref = admitted["workspace_ref"]
        staged_attachment = admitted["attachment_manifest"][0]
        assert staged_attachment["sha256"] == attachment_sha
        attachment_read = broker.prepare(
            workspace_ref,
            request_id=f"request-attachment-{uuid.uuid4().hex}",
            operation_id=f"operation-attachment-{uuid.uuid4().hex}",
            tool="run_command",
            args={
                "cmd": ["cat", staged_attachment["execution_path"]],
                "cwd": _REMOTE_WORKSPACE,
            },
            task_id=task_id,
        )
        attachment_result = broker.execute_prepared(
            workspace_ref,
            attachment_read,
            canonical_args=attachment_read.execution_args,
            task_id=task_id,
        )
        assert attachment_result.process is not None
        assert attachment_result.process.stdout == attachment_payload.decode()
...
def test_rws_107_hardened_forward_rejects_alias_owned_forwards(
    docker_ssh_host,
):
